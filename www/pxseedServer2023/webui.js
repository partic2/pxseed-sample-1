define("pxseedServer2023/webui", ["require", "exports", "preact", "partic2/pComponentUi/texteditor", "partic2/pComponentUi/domui", "partic2/pComponentUi/window", "partic2/pComponentUi/workspace", "partic2/pxprpcClient/registry", "./clientFunction", "partic2/jsutils1/base", "partic2/jsutils1/webutils", "./webentry", "partic2/JsNotebook/workspace"], function (require, exports, React, texteditor_1, domui_1, window_1, workspace_1, registry_1, clientFunction_1, base_1, webutils_1, webentry_1, workspace_2) {
    ...
    class PxseedServerAdministrateTool extends React.Component {
        ...
        async buildEnviron() {
            await alertIfError(async () => {
                let ur = await (0, window_1.confirm)('build environ?');
                if (ur == 'cancel')
                    return;
        ...
        async forceRebuildPackages() {
            await alertIfError(async () => {
                let ur = await (0, window_1.confirm)('force rebuild package?');
                if (ur == 'cancel')
                    return;
        ...
        async restartSubprocess(index) {
            await alertIfError(async () => {
                let ur = await (0, window_1.confirm)('restart subprocess ' + index + '?');
                if (ur == 'cancel')
                    return;
        ...
        async restartServerHostWorker1() {
            await alertIfError(async () => {
                let ur = await (0, window_1.confirm)('restart server host worker?');
                if (ur == 'cancel')
                    return;
    ...
    (async () => {
        if (__name__ == (0, webutils_1.GetJsEntry)()) {
            (0, workspace_1.setBaseWindowView)(React.createElement(PxseedServerAdministrateTool, null));
        }
    })();
});
